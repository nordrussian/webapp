...
class Lecturer(db.Model): # отсюда  https://pythobyte.com/using-sqlalchemy-with-flask-and-postgresql-056e4a45/
    lecturer_id = db.Column(db.Integer, primary_key=False) # id лектора
    name = db.Column(db.String(80), nullable=False) # Имя
    lastname = db.Column(db.String(80), nullable=False) # Имя
    middlename = db.Column(db.String(80), nullable=False) # Отчество
    birthday = db.Column(db.DateTime, nullable=False) # Дата рождения
    ...
    passport_number = db.Column(db.Integer, nullable=False) # Серия и номер паспорта
    passport_who = db.Column(db.Text, nullable=False) # когда и кем выдан
    issued = db.Column(db.DateTime, nullable=False) # Дата выдачи
    ...
    created_at = db.Column(db.DateTime, nullable=False) # Дата создания договора
